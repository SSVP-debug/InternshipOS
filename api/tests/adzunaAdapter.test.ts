...
describe("parseAdzunaListings", () => {
  ...
  // A3.3 (Opportunity Quality — Deadline Handling): the A3.3 audit found
  // Adzuna never publishes an application deadline, and this adapter
  // deliberately hard-codes deadline_date: null rather than inferring
  // one from posted_date or any other field. This test exists to catch
  // an accidental future regression (e.g. someone wiring a wrong field
  // into deadline_date) — it was previously only implied by the broader
  // shape-mapping test above, never asserted on its own.
  it("A3.3: never infers a deadline — deadline_date is always null for every mapped listing", () => {
    const { listings } = parseAdzunaListings(adzunaSampleResponse);

    expect(listings.length).toBeGreaterThan(0);
    for (const l of listings) {
      expect(l.deadline_date).toBeNull();
    }
  });
